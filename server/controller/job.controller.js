  ...
  try {
    ...
    if (!title || !description || !requirements || !location || !type) {
      return res.status(400).json({
        success: false,
        message:
          "Title, description, requirements, location, and type are required",
      });
    }

    // Use company name from request or user's profile
    const finalCompanyName =
      companyName || req.user.profile?.companyName || "Unknown Company";
...
export const getJobById = async (req, res, next) => {
  try {
    const job = await Job.findById(req.params.id)
      .populate(
        "postedBy",
        "name email profile.companyName profile.companyWebsite",
      )
      .populate(
        "applicants.user",
        "name email profile.skills profile.experience",
      );
    ...
    const isOwner =
      req.user && job.postedBy?._id?.toString() === req.user._id.toString();
...
export const applyToJob = async (req, res, next) => {
  try {
    const { coverLetter, resume, assessmentAnswers, violationCount, flagged } =
      req.body;
    const job = await Job.findById(req.params.id);
